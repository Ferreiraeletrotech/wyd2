...
#define OPENLEVELRANKING 110
#define OPENRANKING 118
#define OPENMACROPERGA 119
...
void SendTextServer(const char* Text, const char* String)
{
	...
	sprintf(stMsgWhisper.String, "%s", String);
...
int TMFieldScene::InitializeScene()
{
	...
	if(Composer)
	{
		Composer->SetVisible(0);
	}

	m_pccmode->SetPos(RenderDevice::r_fWidthRatio * 230.0f, RenderDevice::r_fHeightRatio * 523.0f);
	...
	m_pChatList->m_pScrollBar->SetVisible(0);	

	if (g_pDevice->m_dwScreenWidth == 800 || g_pDevice->m_dwScreenWidth == 640 || g_pDevice->m_dwScreenWidth == 1024 || g_pDevice->m_dwScreenWidth == 1280) {
		m_pChatListnotice->m_nPosX = 370.0f * RenderDevice::r_fWidthRatio;
		m_pChatListnotice->m_nPosY = 420.0f * RenderDevice::r_fHeightRatio;
	}
	else {
		m_pChatListnotice->m_nPosX = 550.0f * RenderDevice::r_fWidthRatio;
		m_pChatListnotice->m_nPosY = 520.0f * RenderDevice::r_fHeightRatio;
	}
	...
	if (g_pDevice->m_dwScreenWidth < 1366)
	{
	...
	//centraliza
	m_pMainInfo1->SetAutoSize();
	m_pMainInfo1->m_nPosX = (((float)g_pDevice->m_dwScreenWidth - m_pMainInfo1->m_nWidth) / 2);
	m_pMainInfo1->SetStickBottom();

	//m_pMessagePanel->m_nPosX = (((float)g_pDevice->m_dwScreenWidth - (m_pMessageBox->m_nWidth * RenderDevice::r_fWidthRatio)) / 2.0f);
	...
	m_pChatBrilho = (SButton*)m_pControlContainer->FindControl(190200);//bot�o de fechar o chat
	...
	case 1366:
	{
		m_pChatBrilho->m_nPosY += 170.0f;
		m_pChatList->m_nPosY += 180.0f;
		m_pChatBack->m_nPosY += 180.0f;
		m_pChatPanel->m_nPosY += 180.0f;
		m_pChatListPanel->m_nPosY += 180.0f;
		m_pChatSelectPanel->m_nPosY += 180.0f;
		break;
	}
	case 1600:
	{
		m_pChatBrilho->m_nPosY += 190.0f;
		m_pChatList->m_nPosY += 200.0f;
		m_pChatBack->m_nPosY += 200.0f;
		m_pChatPanel->m_nPosY += 200.0f;
		m_pChatListPanel->m_nPosY += 200.0f;
		m_pChatSelectPanel->m_nPosY += 200.0f;
		break;
	}
	case 1920:
	{
		m_pChatBrilho->m_nPosY += 220.0f;
		m_pChatList->m_nPosY += 230.0f;
		m_pChatBack->m_nPosY += 230.0f;
		m_pChatPanel->m_nPosY += 230.0f;
		m_pChatListPanel->m_nPosY += 230.0f;
		m_pChatSelectPanel->m_nPosY += 230.0f;
		break;
	}
	case 2560:
	{
		m_pChatBrilho->m_nPosY += 240.0f;
		m_pChatList->m_nPosY += 250.0f;
		m_pChatBack->m_nPosY += 250.0f;
		m_pChatPanel->m_nPosY += 250.0f;
		m_pChatListPanel->m_nPosY += 250.0f;
		m_pChatSelectPanel->m_nPosY += 250.0f;
		break;
	}
	}
	...
	m_pMiniPanel->SetPos(RenderDevice::r_fWidthRatio * 427.0f, RenderDevice::r_fHeightRatio * 522.0f);
	...
	m_pInvenPanel->SetPos(RenderDevice::r_fWidthRatio * 514.0f,
		RenderDevice::r_fHeightRatio * 35.0f);
	...
	int Painels[] = { 456654, 1919400, 1882000, 1212300, 121200, 188500, 117900, 179500, 152000, 199300, 981300, 98300, 65686, 65692 };

	if (g_pDevice->m_dwScreenWidth != 800 || g_pDevice->m_dwScreenWidth != 640 || g_pDevice->m_dwScreenWidth != 1024 || g_pDevice->m_dwScreenWidth != 1280) {
		for (int p = 0; p < (sizeof(Painels) / 4); p++) {
			SPanel* ResizeNewPainels = (SPanel*)m_pControlContainer->FindControl(Painels[p]);
			ResizeNewPainels->SetPos(ResizeNewPainels->m_nPosX * RenderDevice::r_fWidthRatio, ResizeNewPainels->m_nPosY * RenderDevice::r_fHeightRatio);
		}
	}

	SPanel* Store = (SPanel*)m_pControlContainer->FindControl(65686);
	SPanel* InvPainel = (SPanel*)m_pControlContainer->FindControl(589832);
	InvPainel->SetPos(Store->m_nPosX + Store->m_nWidth, Store->m_nPosY);
...
#pragma region Novas Janelas
	...
	auto StoreConfirm2 = (SPanel*)m_pControlContainer->FindControl(152000);
	...
	auto InfoPlayer = (SPanel*)m_pControlContainer->FindControl(456654); 

	auto BattlePass = (SPanel*)m_pControlContainer->FindControl(1551000);
	BattlePass->SetVisible(0);

	auto ArenaReal = (SPanel*)m_pControlContainer->FindControl(117800);
	ArenaReal->SetVisible(0);
	
	InfoPlayer->SetVisible(0);
...
int TMFieldScene::OnControlEvent(unsigned int idwControlID, unsigned int idwEvent)
{
	...
	if (idwControlID == B_CCMODE_DLG_MOUNT)
	{
		...
		r_AutoMacroMP = g_GameAuto_mountValue;
	...
	if (idwControlID == B_CCMODE_DLG_HP)
	{
		...
		r_AutoMacroHP = g_GameAuto_hpValue;
	...
	if (idwControlID == P_CCMODE_DLG_PONT)
	{
		...
		r_AutoMacroMove = m_AutoPostionUse;
	...
	if (idwControlID == 642)
	{
		...
		if (m_pMyHuman && (GuildLevel >= 3 && GuildLevel <= 8 || GuildLevel == 9) &&
	...
	if (idwControlID == 863)
	{
		...
		if (GuildLevel == 9 && (m_pPGTOver->m_sGuildLevel < 3 || m_pPGTOver->m_sGuildLevel > 8) &&
	...
	if (idwControlID == 475138)
	{
		...
		if (pPartyItem && pPartyItem->m_nState == 1 && !pPartyList->m_bRButton)
		{
			...
			Reconnect_Conn_id_leader = pPartyItem->m_dwCharID;

			sprintf(stCNFParty2.LeaderName, pPartyItem->GetText());
			sprintf(Reconnect_LeaderName, pPartyItem->GetText());
...
// Novas Janelas - button control
	...
	auto BuyConfirm = (SPanel*)m_pControlContainer->FindControl(152000);
	auto DropList = (SPanel*)m_pControlContainer->FindControl(179500);
	auto PainelPix = (SPanel*)m_pControlContainer->FindControl(188500);
	...
	if (SendHandleID == B_COMMUNITY) //981306
		SendHandleID = OPENDROPLIST;

	if (SendHandleID == 73729)
		SendHandleID = OPENDROPLIST;

	if (SendHandleID == B_HELP) //981310
		SendHandleID = OPENRANKING;

	if (SendHandleID == 1919540)
		SendHandleID = OPENPVPRANKING;

	if (SendHandleID == 1919541)
		SendHandleID = OPENLEVELRANKING;

	if (SendHandleID == 65798) //981308
	...
	if (SendHandleID == 981309) //981309
		SendHandleID = OPEMPIX;

	if (SendHandleID == 65797)
		SendHandleID = OPENMACROPERGA;
	...
	if (SendHandleID >= 55000 && SendHandleID <= 55012) {
		DropListButton = SendHandleID - 55000;
		SendHandleID = DROPLISTBUTTON;
	}
	if (SendHandleID >= 279501 && SendHandleID <= 279510) {
		DropListButton = SendHandleID - 279501;
		SendHandleID = DROPLISTNAMES;
	}
	...
	case 181923: {
		...
		stReqNPC.Type = _MSG_ReqNPC;
		stReqNPC.ID = m_pMyHuman->m_dwID;
		stReqNPC.State = 2;
		stReqNPC.Comp = g_pClientInfo->Info.Composition;
		stReqNPC.NPCID = g_pClientInfo->Info.Composer;

		SendOneMessage((char*)&stReqNPC, sizeof(stReqNPC));

		auto Composer = (SPanel*)m_pControlContainer->FindControl(181921);
		Composer->SetVisible(0);
	}break;
#pragma region Passe de Batalha
	case 1551100: //BattlePass Anterior
	{
		int MaxPages = 3;

		int Page = g_pClientInfo->cConfigR.bPassPage;
		if (Page > 0) {
			ReqAlias(200 + Page - 1);
			g_pClientInfo->cConfigR.bPassPage--;
		}
		else {
			Page = MaxPages - 2;
			ReqAlias(200 + Page + 1);
	...
	case 1551101: //BattlePass Pr�ximo
	{
		int MaxPages = 3;
		...
		if (Page < MaxPages - 1) {
			...
			Page = -1;
			ReqAlias(200 + Page + 1);
			g_pClientInfo->cConfigR.bPassPage = 0;
		}
	}break;
	case 1551102: {
		ReqAlias(19);
...
#pragma endregion
	case OPENMACROPERGA: {
		MacroUP = !MacroUP;
		char str[128] = {};
		sprintf(str, "Macro Perga : %s", (MacroUP ? "ON" : "OFF"));
	...
	{
		char Name[16];
		auto pEditID = (SText*)m_pControlContainer->FindControl(456610);
		
		strncpy(Name, pEditID->GetText(), 16);
		SendTextServer("infoplayer", Name);		
	}break;
	case B_SYSTEM: //981311
	{
		for (int x = 1; x < 18; x++) {
			auto EquipGrid = (SGridControl*)m_pControlContainer->FindControl(21000 + x);
			EquipGrid->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;
			EquipGrid->Empty();
		}

		auto Name = (SText*)m_pControlContainer->FindControl(456600);
		Name->SetText("Name:", 0);
		auto Evo = (SText*)m_pControlContainer->FindControl(456601);
		Evo->SetText("Evo:", 0);
		auto Classe = (SText*)m_pControlContainer->FindControl(445602);
		Classe->SetText("Class:", 0);
		auto Guilda = (SText*)m_pControlContainer->FindControl(456603);
		Guilda->SetText("Guild:", 0);

		auto InfoName = (SText*)m_pControlContainer->FindControl(456604);
		InfoName->SetText("", 0);

		auto InfoNIVEL = (SText*)m_pControlContainer->FindControl(456609);
		InfoNIVEL->SetText("", 0);

		auto InfoEVO = (SText*)m_pControlContainer->FindControl(456605);
		InfoEVO->SetText("", 0);

		auto InfoClass = (SText*)m_pControlContainer->FindControl(456606);
		InfoClass->SetText("", 0);

		auto InfoGuild = (SText*)m_pControlContainer->FindControl(456607);
		InfoGuild->SetText("", 0);

		auto InfoPlayer = (SPanel*)m_pControlContainer->FindControl(456654);
		InfoPlayer->SetVisible(1);
	...
	case 1919430: //Ranking - Anterior
	{
		if (g_pClientInfo->Ranking.State == 1) {
			if (g_pClientInfo->cConfigR.RankingPage >= 2) {
				...
				auto lPos1 = (SText*)m_pControlContainer->FindControl(1919710);
				auto lPos2 = (SText*)m_pControlContainer->FindControl(1919711);
				auto lPos3 = (SText*)m_pControlContainer->FindControl(1919712);
				auto lPos4 = (SText*)m_pControlContainer->FindControl(1919713);
				auto lPos5 = (SText*)m_pControlContainer->FindControl(1919714);
				auto lPos6 = (SText*)m_pControlContainer->FindControl(1919715);
				auto lPos7 = (SText*)m_pControlContainer->FindControl(1919716);
				auto lPos8 = (SText*)m_pControlContainer->FindControl(1919717);
				auto lPos9 = (SText*)m_pControlContainer->FindControl(1919718);
				auto lPos10 = (SText*)m_pControlContainer->FindControl(1919719);
				...
				Pos1->SetText(strFmt("%d. %s", 1 + Pos, g_pClientInfo->Ranking.RankName[0 + Pos]), 0);
				Pos2->SetText(strFmt("%d. %s", 2 + Pos, g_pClientInfo->Ranking.RankName[1 + Pos]), 0);
				Pos3->SetText(strFmt("%d. %s", 3 + Pos, g_pClientInfo->Ranking.RankName[2 + Pos]), 0);
				Pos4->SetText(strFmt("%d. %s", 4 + Pos, g_pClientInfo->Ranking.RankName[3 + Pos]), 0);
				Pos5->SetText(strFmt("%d. %s", 5 + Pos, g_pClientInfo->Ranking.RankName[4 + Pos]), 0);
				Pos6->SetText(strFmt("%d. %s", 6 + Pos, g_pClientInfo->Ranking.RankName[5 + Pos]), 0);
				Pos7->SetText(strFmt("%d. %s", 7 + Pos, g_pClientInfo->Ranking.RankName[6 + Pos]), 0);
				Pos8->SetText(strFmt("%d. %s", 8 + Pos, g_pClientInfo->Ranking.RankName[7 + Pos]), 0);
				Pos9->SetText(strFmt("%d. %s", 9 + Pos, g_pClientInfo->Ranking.RankName[8 + Pos]), 0);
				Pos10->SetText(strFmt("%d. %s", 10 + Pos, g_pClientInfo->Ranking.RankName[9 + Pos]), 0);

				lPos1->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[0 + Pos]), 0);
				lPos2->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[1 + Pos]), 0);
				lPos3->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[2 + Pos]), 0);
				lPos4->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[3 + Pos]), 0);
				lPos5->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[4 + Pos]), 0);
				lPos6->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[5 + Pos]), 0);
				lPos7->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[6 + Pos]), 0);
				lPos8->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[7 + Pos]), 0);
				lPos9->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[8 + Pos]), 0);
				lPos10->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[9 + Pos]), 0);


				for (int i = 0; i < 10; i++) {
					auto cape = new STRUCT_ITEM;
					memset(cape, 0, sizeof(STRUCT_ITEM));

					auto classe = new STRUCT_ITEM;
					memset(classe, 0, sizeof(STRUCT_ITEM));

					auto evolu = new STRUCT_ITEM;
					memset(evolu, 0, sizeof(STRUCT_ITEM));

					// Capa
					auto Reino = (SGridControl*)m_pControlContainer->FindControl(1919530 + i);
					Reino->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;
					Reino->Empty();
					cape->sIndex = 5809;

					if (g_pClientInfo->Ranking.Kingdom[i + Pos] == 7 || g_pClientInfo->Ranking.Kingdom[i + Pos] == 8) {
						cape->sIndex = g_pClientInfo->Ranking.Kingdom[i + Pos] + 5800;
					}

					if (cape->sIndex != 0) {
						auto pItem = new SGridControlItem(0, cape, 0.0f, 0.0f);
						pItem->m_nWidth = 20.0f * RenderDevice::m_fWidthRatio;
						pItem->m_nHeight = 20.0f * RenderDevice::m_fHeightRatio;
						pItem->m_GCObj.m_fWidth = 20.0f * RenderDevice::m_fWidthRatio;
						pItem->m_GCObj.m_fHeight = 20.0f * RenderDevice::m_fHeightRatio;
						Reino->AddItem(pItem, 0, 0);
					}

					// Classe
					auto ClassMaster = (SGridControl*)m_pControlContainer->FindControl(1919520 + i);
					ClassMaster->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;
					ClassMaster->Empty();


					classe->sIndex = g_pClientInfo->Ranking.RankClasse[i + Pos] + 5800;

					if (classe->sIndex != 0) {
						auto pItem2 = new SGridControlItem(0, classe, 0.0f, 0.0f);
						pItem2->m_nWidth = 20.0f * RenderDevice::m_fWidthRatio;
						pItem2->m_nHeight = 20.0f * RenderDevice::m_fHeightRatio;
						pItem2->m_GCObj.m_fWidth = 20.0f * RenderDevice::m_fWidthRatio;
						pItem2->m_GCObj.m_fHeight = 20.0f * RenderDevice::m_fHeightRatio;
						ClassMaster->AddItem(pItem2, 0, 0);
					}

					// Evolution
					auto Evolution = (SGridControl*)m_pControlContainer->FindControl(1919510 + i);
					Evolution->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;
					Evolution->Empty();

					if (g_pClientInfo->Ranking.RankEvolution[i + Pos] == 1)
						evolu->sIndex = 5804;
					if (g_pClientInfo->Ranking.RankEvolution[i + Pos] == 2)
						evolu->sIndex = 5805;
					if (g_pClientInfo->Ranking.RankEvolution[i + Pos] >= 3)
						evolu->sIndex = 5806;

					if (evolu->sIndex != 0) {
						auto pItem3 = new SGridControlItem(0, evolu, 0.0f, 0.0f);
						pItem3->m_nWidth = 20.0f * RenderDevice::m_fWidthRatio;
						pItem3->m_nHeight = 20.0f * RenderDevice::m_fHeightRatio;
						pItem3->m_GCObj.m_fWidth = 20.0f * RenderDevice::m_fWidthRatio;
						pItem3->m_GCObj.m_fHeight = 20.0f * RenderDevice::m_fHeightRatio;
						Evolution->AddItem(pItem3, 0, 0);
					}
				}
			}
		}
		if (g_pClientInfo->Ranking.State == 0) {
			if (g_pClientInfo->cConfigR.RankingPage >= 2) {
				...
				lPos1->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[0 + Pos]), 0);
				lPos2->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[1 + Pos]), 0);
				lPos3->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[2 + Pos]), 0);
				lPos4->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[3 + Pos]), 0);
				lPos5->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[4 + Pos]), 0);
				lPos6->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[5 + Pos]), 0);
				lPos7->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[6 + Pos]), 0);
				lPos8->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[7 + Pos]), 0);
				lPos9->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[8 + Pos]), 0);
				lPos10->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[9 + Pos]), 0);
	...
	case JEPHIBUTTON:
	{
		if (g_pClientInfo->cConfigR.WindowControl == JEPHI) {
			if (JephiButton == 0 || JephiButton == 1) {
				...
				auto BuyConfirmGrid = (SGridControl*)m_pControlContainer->FindControl(152007);
	...
	case OPENDROPLIST:
	{
		...
		auto NewWindowTX11 = (SText*)m_pControlContainer->FindControl(179521);
		auto NewWindowTX12 = (SText*)m_pControlContainer->FindControl(179522);
		auto NewWindowTX13 = (SText*)m_pControlContainer->FindControl(179523);
		...
		NewWindowTX9->SetText(strFmt("Carta N"), 0);
		NewWindowTX10->SetText(strFmt("Carta M"), 0);
		NewWindowTX11->SetText(strFmt("Carta A"), 0);
		NewWindowTX12->SetText(strFmt("Vale"), 0);
		NewWindowTX13->SetText(strFmt("Pista"), 0);

		auto PageTx = (SText*)m_pControlContainer->FindControl(279521);
	...
	case 279531:
	{
		...
		if (g_pClientInfo->cConfigR.DropListRegion == 0) {
			...
			auto NewWindowTX1 = (SText*)m_pControlContainer->FindControl(279511);
			auto NewWindowTX2 = (SText*)m_pControlContainer->FindControl(279512);
			auto NewWindowTX3 = (SText*)m_pControlContainer->FindControl(279513);
			auto NewWindowTX4 = (SText*)m_pControlContainer->FindControl(279514);
			auto NewWindowTX5 = (SText*)m_pControlContainer->FindControl(279515);
			auto NewWindowTX6 = (SText*)m_pControlContainer->FindControl(279516);
			auto NewWindowTX7 = (SText*)m_pControlContainer->FindControl(279517);
			auto NewWindowTX8 = (SText*)m_pControlContainer->FindControl(279518);
			auto NewWindowTX9 = (SText*)m_pControlContainer->FindControl(279519);
			auto NewWindowTX10 = (SText*)m_pControlContainer->FindControl(279520);

			NewWindowTX1->SetText(strFmt(""), 0);
			NewWindowTX2->SetText(strFmt(""), 0);
	...
	case DONATEBUTTON:
	{
		...
		auto ConfirmTx2 = (SText*)m_pControlContainer->FindControl(152001);
		ConfirmTx2->SetText(strFmt("%d", Price), 0);

		if (Price >= g_pClientInfo->Info.Cash)
			ConfirmTx2->SetTextColor(TNColor::Red);
		else
			ConfirmTx2->SetTextColor(TNColor::GreenYellow);

		auto ConfirmTx3 = (SText*)m_pControlContainer->FindControl(152002);
		ConfirmTx3->SetText(strFmt("Atual: %d", g_pClientInfo->Info.Cash), 0);
		ConfirmTx3->SetTextColor(TNColor::GreenYellow);

		auto ConfirmTx4 = (SText*)m_pControlContainer->FindControl(152003);
		ConfirmTx4->SetText(strFmt("Final: %d", (g_pClientInfo->Info.Cash - Price)), 0);
		ConfirmTx4->SetTextColor(TNColor::GreenYellow);
	...
	case 152005:
	{
		if (g_pClientInfo->cConfigR.WindowControl == DONATESTORE) {
			...
			if (g_pClientInfo->cConfigR.ShopStore == 6)
				ReqAlias(2);
		}
		if (g_pClientInfo->cConfigR.WindowControl == JEPHI) {
	...
	case OPENSTORE:
	{
		...
		TxShopPage->SetText(strFmt("P�GINA %d/%d", (g_pClientInfo->cConfigR.ShopPage + 1), tPages), 0);
		...
		for (int i = 0; i < 15; i++) {
			...
			TxShopGrid->SetText(strFmt("%d", g_pClientInfo->Info.cStoreItens[g_pClientInfo->cConfigR.ShopStore][g_pClientInfo->cConfigR.ShopPage][i][1]), 0);
			TxShopGrid->SetTextColor(TNColor::GreenYellow);
	...
	case 199553:
	{
		g_pClientInfo->cConfigR.ShopStore = 3;
		g_pClientInfo->cConfigR.ShopPage = 0;
		GamePainel->SetVisible(0);
		StorePainel->SetVisible(1);
	...
	case 199554:
	{
		g_pClientInfo->cConfigR.ShopStore = 4;
		g_pClientInfo->cConfigR.ShopPage = 0;
		GamePainel->SetVisible(0);
		StorePainel->SetVisible(1);
	...
	case 199555:
	{
		g_pClientInfo->cConfigR.ShopStore = 5;
		g_pClientInfo->cConfigR.ShopPage = 0;
		GamePainel->SetVisible(0);
		StorePainel->SetVisible(1);
	...
	case 199556:
	{
		g_pClientInfo->cConfigR.ShopStore = 6;
		g_pClientInfo->cConfigR.ShopPage = 0;
		GamePainel->SetVisible(0);
		StorePainel->SetVisible(1);
	...
	case 199557:
	{
		g_pClientInfo->cConfigR.ShopStore = 7;
		g_pClientInfo->cConfigR.ShopPage = 0;
		GamePainel->SetVisible(0);
		StorePainel->SetVisible(1);
	...
	case 199558:
	{
		g_pClientInfo->cConfigR.ShopStore = 8;
		g_pClientInfo->cConfigR.ShopPage = 0;
		GamePainel->SetVisible(0);
		StorePainel->SetVisible(1);
	...
	case 199559:
	{
		g_pClientInfo->cConfigR.ShopStore = 9;
		g_pClientInfo->cConfigR.ShopPage = 0;
		GamePainel->SetVisible(0);
		StorePainel->SetVisible(1);
...
#define VK_R 82
	...
	auto pInfoPlayer = (SEditableText*)m_pControlContainer->FindControl(456610);

	if (m_pEditChat->IsFocused() == false && pEdit->IsFocused() == false && pPix->IsFocused() == false && pInfoPlayer->IsFocused() == false)
	{
		if (iKeyCode == VK_F)
		{
			auto painelteleport = (SPanel*)m_pControlContainer->FindControl(98300);
		...
		if (iKeyCode == VK_D)
		{
			...
			DropList->SetVisible(DropList->m_bVisible == 0);
			g_pClientInfo->cConfigR.DropListPage = 0;
			g_pClientInfo->cConfigR.DropListRegion = 0;
		...
		if (iKeyCode == VK_B) {			
			auto BattlePass = (SPanel*)m_pControlContainer->FindControl(1551000);
			bool visible = BattlePass->IsVisible();
			if (visible == true)
				BattlePass->SetVisible(0);

			if (visible == false) {
				BattlePass->SetVisible(1);
				g_pClientInfo->cConfigR.bPassPage = 0;
				ReqAlias(200);
			}
		}
...
int TMFieldScene::OnPacketEvent(unsigned int dwCode, char* buf)
{
	...
	case _MSG_UpdateFilterList:
	{
		auto FilterList = (SGridControl*)m_pControlContainer->FindControl(279600);

		if (!FilterList)
			return 0;

		if (FilterList->m_nNumItem)
		{
			FilterList->Empty();
		}

		auto pUpdateFilterList = reinterpret_cast<MSG_UpdateFilterList*>(pStd);
		...
		for (int i = 0; i < 16; i++) {
			...
			if (pUpdateFilterList->Item[i] == 0)
				continue;
			std::cout << "\n pUpdateFilterList->Item[i] " << pUpdateFilterList->Item[i];
			auto item = new STRUCT_ITEM;
			item->sIndex = pUpdateFilterList->Item[i];
			...
			auto pItem = new SGridControlItem(0, item, 0.0f, 0.0f);
			FilterList->AddItem(pItem, i % 8, i / 8);
		}
	...
	case _MSG_RequestInfoPlay:
		return OnPacketSendInfoPlayer(pStd);
	case _MSG_ReconnectGrupo:
		return OnPacketReconnectParty(pStd);
	case _MSG_SendBattlePass:
		return OnPacketSendBattlePass(pStd);
	case _MSG_SendArenaReal:
		return OnPacketSendArenaReal(pStd);
	case _MSG_SendNPC:
		return OnPacketSendNpc(pStd);
		break;
	}
...
int TMFieldScene::CheckItemQntCarry(STRUCT_ITEM Item1) {
	auto player = g_pObjectManager->m_stMobData;

	int sQNT = 0;

	for (int i = 0; i < MAX_CARRY; i++) {
		if (player.Carry[i].sIndex == Item1.sIndex) {
			if (player.Carry[i].stEffect[0].cEffect == 61) {
				sQNT += player.Carry[i].stEffect[0].cValue;
			}
			else if (player.Carry[i].stEffect[1].cEffect == 61) {
				sQNT += player.Carry[i].stEffect[1].cValue;
			}
			else if (player.Carry[i].stEffect[1].cEffect == 61) {
				sQNT += player.Carry[i].stEffect[1].cValue;
			}
...
int TMFieldScene::CheckSameItemCarry(STRUCT_ITEM Item1) {

	auto player = g_pObjectManager->m_stMobData;

	for (int i = 0; i < MAX_CARRY; i++) {
		if (player.Carry[i].sIndex == 0)
			continue;

		if (memcmp(&player.Carry[i], &Item1, sizeof(STRUCT_ITEM)) != TRUE) {
			return TRUE;
		}
	}
	return FALSE;
...
int TMFieldScene::OnPacketSendNpc(MSG_STANDARD* pStd)
{
	auto p = reinterpret_cast<MSG_SendNPC*>(pStd);
	...
	if (p->State == 0) {
		for (int i = 0; i < 16; i++) {
			//Grids de cima
			auto Comps = (SGridControl*)m_pControlContainer->FindControl(181924 + i);
			Comps->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;
			Comps->Empty();
			...
			memcpy(item2, &p->Item[i], sizeof(STRUCT_ITEM));

			auto pItem2 = new SGridControlItem(0, item2, 0.0f, 0.0f);
			Comps->AddItem(pItem2, 0, 0);
		}
	}
	if (p->State == 1) {
		...
		for (int i = 0; i < 8; i++) {
			//Grids de baixo
			auto Comps = (SGridControl*)m_pControlContainer->FindControl(181981 + i);
			Comps->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;
			Comps->Empty();

			auto Desc = (SText*)m_pControlContainer->FindControl(181959 + i);
			Desc->SetText("", 0);
			...
			if (p->Item[i].stEffect[0].cEffect == 61) {
				QNT = p->Item[i].stEffect[0].cValue;

				int nQNT = CheckItemQntCarry(p->Item[i]);

				Desc->SetText(strFmt("%d/%d", nQNT, QNT), 0);
				Desc->SetTextColor(TNColor::Red);

				if (nQNT >= QNT)
					Desc->SetTextColor(TNColor::Speak);
			}
			else {
				...
				for (int x = 0; x < MAX_CARRY; x++) {
					...
					if (memcmp(&player.Carry[x], &p->Item[i], sizeof(STRUCT_ITEM)))
						continue;
		...
		std::string sCoin;
		std::string sCoinF;
		sCoin = strFmt("%d", p->Gold[0]);
		sCoinF = quickAddThousandSeparators(sCoin);

		char Porcentagem[2] = "%";

		auto Gold = (SText*)m_pControlContainer->FindControl(181949);
		Gold->SetText(strFmt("%s", sCoinF.c_str()), 0);
		Gold->SetTextColor(TNColor::GoldenClaro);

		char Prc[2] = { "%" };

		auto Rate = (SText*)m_pControlContainer->FindControl(181948);
		Rate->SetText(strFmt("Rate: %d%s", p->Gold[1], Prc), 0);
	}

	return 1;
}

int TMFieldScene::OnPacketSendArenaReal(MSG_STANDARD* pStd) {
	auto p = reinterpret_cast<MSG_SendArenaReal*>(pStd);

	auto ArenaReal = (SPanel*)m_pControlContainer->FindControl(117800);
	ArenaReal->SetVisible(1);

	auto Equip1 = (SText*)m_pControlContainer->FindControl(117801);
	Equip1->SetText(strFmt("%d", p->Kills[0]), 0);
	Equip1->SetTextColor(TNColor::DeepPink);

	auto Equip2 = (SText*)m_pControlContainer->FindControl(117802);
	Equip2->SetText(strFmt("%d", p->Kills[1]), 0);
	Equip2->SetTextColor(TNColor::DarkOrange);

	auto Equip3 = (SText*)m_pControlContainer->FindControl(117803);
	Equip3->SetText(strFmt("%d", p->Kills[2]), 0);
	Equip3->SetTextColor(TNColor::Speak);

	auto Equip4 = (SText*)m_pControlContainer->FindControl(117804);
	Equip4->SetText(strFmt("%d", p->Kills[3]), 0);
	Equip4->SetTextColor(TNColor::CornBlue);

	return 1;
}


int TMFieldScene::OnPacketSendBattlePass(MSG_STANDARD* pStd)
{
	auto p = reinterpret_cast<MSG_SendBattlePass*>(pStd);
	auto player = g_pObjectManager->m_stMobData;
	...
	g_pClientInfo->cConfigR.bPassPage = Page;
	int LvStat = Level - (Page * 10);

	auto Progress = (SProgressBar*)m_pControlContainer->FindControl(1551400);
	Progress->SetMaxProgress(100000);
	Progress->SetCurrentProgress(Points);

	auto dLevel = (SText*)m_pControlContainer->FindControl(1551300);
	dLevel->SetText(strFmt("Level %d", Level + 1), 0);
	dLevel->SetTextColor(TNColor::GoldenClaro);

	std::string sPoints;
	std::string fPoints;
	sPoints = strFmt("%d", Points);
	fPoints = quickAddThousandSeparators(sPoints);

	auto dPoints = (SText*)m_pControlContainer->FindControl(1551301);
	dPoints->SetText(strFmt("Points: %s / 100,000", fPoints.c_str()), 0);
	dPoints->SetTextColor(TNColor::GoldenClaro);

	auto dPage = (SText*)m_pControlContainer->FindControl(1551303);
	dPage->SetText(strFmt("Page: %d", p->Page + 1), 0);
	dPage->SetTextColor(TNColor::GoldenClaro);

	char Prc[2] = { "%" };
	int nPrc = (int)(Points / 1000);

	auto dProgress = (SText*)m_pControlContainer->FindControl(1551302);
	dProgress->SetText(strFmt("Progress: %d%s", nPrc, Prc), 0);
	dProgress->SetTextColor(TNColor::GoldenClaro);

	auto TaskListBox = (SListBox*)m_pControlContainer->FindControl(1551500);
	LoadMsgText2(TaskListBox, (char*)"UI\\BPass.txt", 0, 8);

	auto TaskListBox2 = (SListBox*)m_pControlContainer->FindControl(155501);
	LoadMsgText2(TaskListBox2, (char*)"UI\\BPass.txt", 10, 19);

	for (int i = 0; i < 10; i++) {
		auto Itens = (SGridControl*)m_pControlContainer->FindControl(1551200 + i);
		Itens->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;
		Itens->Empty();

		auto Button = (SButton*)m_pControlContainer->FindControl(1551110 + i);
		Button->SetTextureSetIndex(-2);

		if (p->Item[i].sIndex == 0)
			continue;

		auto item = new STRUCT_ITEM;
		memcpy(item, &p->Item[i], sizeof(STRUCT_ITEM));

		auto pItem = new SGridControlItem(0, item, 0.0f, 0.0f);
		Itens->AddItem(pItem, 0, 0);

		if (LvStat > i) {
			Button->SetTextureSetIndex(658);
		}
	}
	return 1;
}

int TMFieldScene::OnPacketReconnectParty(MSG_STANDARD* pStd)
{
	auto p = reinterpret_cast<MSG_ReconnectGrupo*>(pStd);

	Reconnect_Conn_id_leader = p->Index;
	sprintf(Reconnect_LeaderName, p->Nick);

	if (ReconnectState == 4) {
		m_pMessagePanel->SetMessage("Verificando Grupo...", 4000);
		m_pMessagePanel->SetVisible(1, 2);

		auto pPartyList = m_pPartyList;
		auto pPartyItem = (SListBoxPartyItem*)pPartyList->GetItem(0);

		{
			MSG_CNFParty2 stCNFParty2{};
			stCNFParty2.Header.ID = m_pMyHuman->m_dwID;
			stCNFParty2.Header.Type = MSG_CNFParty2_Opcode;
			stCNFParty2.LeaderID = Reconnect_Conn_id_leader;

			sprintf(stCNFParty2.LeaderName, Reconnect_LeaderName);


			auto pHuman = (TMHuman*)g_pObjectManager->GetHumanByID(Reconnect_Conn_id_leader);

			if (pHuman)
			{

				if (Reconnect_Conn_id_leader > 0 && Reconnect_Conn_id_leader < 1000)
				{
					pHuman->m_bParty = 1;
				}
				else
...
int TMFieldScene::FrameMove(unsigned int dwServerTime)
{
	...
	for (int i = 0; i < 32; ++i)
	{
		if ((unsigned char)m_pMyHuman->m_stAffect[i].Type > 0)
		{
			...
			if (m_pMiniPanel)
			{
				if ((unsigned char)m_pMyHuman->m_stAffect[i].Type < 99)
	...
	if (m_pRankTimeText && m_bRankTimeOn) //Greentime
	{
		if (m_pGround->m_vecOffsetIndex.x == 1 && m_pGround->m_vecOffsetIndex.y == 31
			...
			|| m_pGround->m_vecOffsetIndex.x == 28 && m_pGround->m_vecOffsetIndex.y == 30 //Lanhouse A
			|| m_pGround->m_vecOffsetIndex.x == 29 && m_pGround->m_vecOffsetIndex.y == 27 //Lanhouse M
			|| m_pGround->m_vecOffsetIndex.x == 28 && m_pGround->m_vecOffsetIndex.y == 28 //Lanhouse N
			|| (m_pGround->m_vecOffsetIndex.x == 26 || m_pGround->m_vecOffsetIndex.x == 27) &&  //
			(m_pGround->m_vecOffsetIndex.y == 8 || m_pGround->m_vecOffsetIndex.y == 10 ||       // Pista de Runas
			m_pGround->m_vecOffsetIndex.y == 11 || m_pGround->m_vecOffsetIndex.y == 12 )        // 
...
int TMFieldScene::MouseClick_NPC(int nX, int nY, D3DXVECTOR3 vec, unsigned int dwServerTime)
{
	...
	if (pOver->m_dwID >= 1000 && (pOver->m_stScore.Reserved & 0xF) == 1)
	{
		...
		if (strcmp(pOver->m_szName, "Missao I") == 0) {
			...
			g_pClientInfo->Info.Composer = 0; //numero do npc
		...
		if (strcmp(pOver->m_szName, "Missao II") == 0) {
			auto Composer = (SPanel*)m_pControlContainer->FindControl(181921);
			Composer->SetVisible(1);
			...
			g_pClientInfo->Info.Composer = 1; //numero do npc
		...
		if (strcmp(pOver->m_szName, "Missao III") == 0) {
			auto Composer = (SPanel*)m_pControlContainer->FindControl(181921);
			Composer->SetVisible(1);
			...
			g_pClientInfo->Info.Composer = 2; //numero do npc
		...
		if (strcmp(pOver->m_szName, "LootMaster") == 0) {
			auto Composer = (SPanel*)m_pControlContainer->FindControl(181921);
			Composer->SetVisible(1);
			...
			g_pClientInfo->Info.Composer = 3; //numero do npc
		...
		if (strcmp(pOver->m_szName, "Exchange") == 0) {
			auto Composer = (SPanel*)m_pControlContainer->FindControl(181921);
			Composer->SetVisible(1);
			...
			g_pClientInfo->Info.Composer = 4; //numero do npc
		...
		if (strcmp(pOver->m_szName, "Joalheiro") == 0) {
			auto Composer = (SPanel*)m_pControlContainer->FindControl(181921);
			Composer->SetVisible(1);
			...
			g_pClientInfo->Info.Composer = 5; //numero do npc
		...
		if (strcmp(pOver->m_szName, "Ticket Premium") == 0) {
			auto Composer = (SPanel*)m_pControlContainer->FindControl(181921);
			Composer->SetVisible(1);
			...
			g_pClientInfo->Info.Composer = 6; //numero do npc
		...
		if (pOver->m_sHeadIndex == 58 && pOver->m_sHelmIndex == 115) {
			...
			TxShopGrid->SetText(strFmt("Click on the chosen item to compose"), 0);
...
void TMFieldScene::SetVisibleShop(int bShow)
{
	...
	if (bShow)
		m_pInvenPanel->SetPos(RenderDevice::r_fWidthRatio * 514.0f, RenderDevice::r_fHeightRatio * 35.0f);
...
void TMFieldScene::SetVisibleCargo(int bShow)
{
	...
	{
		m_pInvenPanel->SetPos(RenderDevice::r_fWidthRatio * 514.0f, RenderDevice::r_fHeightRatio * 35.0f);
		m_pCargoPanel->SetPos(RenderDevice::r_fWidthRatio * 287.0f, RenderDevice::r_fHeightRatio * 35.0f);
		m_pCargoPanel1->SetPos(RenderDevice::r_fWidthRatio * 287.0f, RenderDevice::r_fHeightRatio * 35.0f);
	}
...
void TMFieldScene::SetVisibleCargo1(int bShow)
{
	...
	if (bShow)
	{
		pPanel->SetPos(RenderDevice::r_fWidthRatio * 514.0f,
			RenderDevice::r_fHeightRatio * 35.0f);
		m_pCargoPanel1->SetPos(RenderDevice::r_fWidthRatio * 287.0f,
			RenderDevice::r_fHeightRatio * 35.0f);
	}
...
void TMFieldScene::OnESC()
{
	...
	else if (InfoPlayer->IsVisible() == 1) {

		for (int i = 1; i < 16; i++) {
			auto Equip = (SGridControl*)m_pControlContainer->FindControl(21001 + i);
			Equip->Empty();
		}
		InfoPlayer->SetVisible(0);
	}
	...
	else if (ArenaReal->IsVisible() == 1) {
		ArenaReal->SetVisible(0);
	}
	else if (Composer->IsVisible() == 1) {
		for (int i = 0; i < 16; i++) {
			//Grids de cima
			auto Comps = (SGridControl*)m_pControlContainer->FindControl(181924 + i);
			Comps->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;
	...
	if (m_pAutoTrade)
	{
		...
		if (bShow == 1)
		{
			...
			if (bCargo == 1)
			{
				...
				//m_pCargoPanel1->SetPos(RenderDevice::m_fWidthRatio * 514.0f, RenderDevice::m_fHeightRatio * 35.0f); //Corre�o Lojinha
...
char TMFieldScene::UseMPotion()
{
	...
	if (BASE_GetItemAbility(pItem->m_pItem, 38) == 1)
	{
		...
		if (m_dwUseItemTime && dwServerTime - m_dwUseItemTime < 750)
			return 0;
...
int TMFieldScene::OnPacketMessageWhisper(MSG_MessageWhisper* pMsg)
{
	...
	if (strcmp(pMsg->MobName, "[SERVIDOR]") == 0)
	{
		char szMsg2[128]{};
		char szMsg3[128]{};
	...
	if (strlen(pMsg->MobName) + strlen(pMsg->String) <= 45)
	{
		sprintf(szMsg, "[%s]> %s", pMsg->MobName, &pMsg->String[nIndex]);
	...
	else
	{
		...
		if (IsClearString(pMsg->String, 44))
		{
			strncpy(szMsg3, pMsg->String, 45);
			sprintf(szMsg2, "%s", &pMsg->String[45]);
		}
		else
		{
			strncpy(szMsg3, pMsg->String, 44);
			sprintf(szMsg2, "%s", &pMsg->String[44]);
		}
		...
		if (strlen(pMsg->String) > 45 && pChatList)
			pChatList->AddItem(new SListBoxItem(szMsg2, dwColor, 0.0f, 0.0f, 280.0f, 16.0f, 0, 0x77777777, 1, 0));
	}
...
int TMFieldScene::OnPacketCreateMob(MSG_STANDARD* pStd)
{
	...
	if (!pNode)
	{
		...
		if (pCreateMob->MobID > 0 && pCreateMob->MobID < 1000 || pCreateMob->MobID > 25000)
		{
		...
		if ((pCreateMob->Equip[14] & 0xFFF) && pCreateMob->Equip2[14] && (pCreateMob->Equip2[14] & 0xFFF) == 3998)
		{
			pHuman->SetMountCostume(3998);
		}
	...
	if (pHuman)
	{
		...
		else if (pCreateMob->MobID > 25000)
		{
			sprintf(pHuman->m_TradeDesc, "#VEMPROWYD");
			pHuman->m_pAutoTradeDesc->SetText(pHuman->m_TradeDesc, 0);

			pHuman->m_TradeDesc[0] = 1;

			pHuman->m_pAutoTradeDesc->SetVisible(1);
			pHuman->m_pAutoTradePanel->SetVisible(1);
		}
		else
		{
...
int TMFieldScene::OnPacketUpdateDonateStore(MSG_STANDARD* pStd)
{
	...
	if (g_pClientInfo->cConfigR.ShopStore == 6) {
		...
		for (int i = 0; i < 15; i++) {
			...
			auto TxShopGrid = (SText*)m_pControlContainer->FindControl(199501 + i);
			TxShopGrid->SetText(strFmt("Price: %d / Qnt %d", g_pClientInfo->Info.cStoreItens[g_pClientInfo->cConfigR.ShopStore][g_pClientInfo->cConfigR.ShopPage][i][1],
				g_pClientInfo->Info.cStoreItens[g_pClientInfo->cConfigR.ShopStore][g_pClientInfo->cConfigR.ShopPage][i][2]), 0);
			TxShopGrid->SetTextColor(TNColor::CornBlue);
...
int TMFieldScene::OnPacketUpdateRecaptcha(MSG_STANDARD* pStd)
{
	...
	char Keywords[30][11] = {
		"BIKE", "AIRPLANE", "BIKE", "CAR", "SHIP",
		"CAR", "BIKE", "AIRPLANE", "MOTORCYCLE", "MOTO",
		"BIKE", "BIKE", "AIRPLANE", "TRAIN", "BIKE",
		"AIRPLANE", "BIKE", "CAR", "SHIP", "SHIP",
		"SHIP", "MOTORCYCLE", "TRAIN", "TRAIN", "AIRPLANE",
		"TRAIN", "SHIP", "AIRPLANE", "SHIP", "MOTORCYCLE"
	};

	strncpy(Word, Keywords[p->word], 11);

	auto PainelCaptcha = (SPanel*)m_pControlContainer->FindControl(1319001);
	PainelCaptcha->SetVisible(1);

	auto Desc = (SText*)m_pControlContainer->FindControl(1319002);
	Desc->SetText(strFmt("Click on the image that represents a: [%s]", Word), 0);

	auto Desc2 = (SText*)m_pControlContainer->FindControl(1319003);
	Desc2->SetText(strFmt("[ATTENTION] The next attempt will only be in 2 minutes"), 0);
...
int TMFieldScene::OnPacketSendRanking(MSG_STANDARD* pStd)
{
	...
	for (int i = 0; i < 50; i++) {
		...
		g_pClientInfo->Ranking.Kingdom[i] = p->Kingdom[i];		
	...
	if (p->State == 0) {
		...
		Evo->SetText(strFmt("Evo"), 0);
		auto Class = (SText*)m_pControlContainer->FindControl(1919405);
		Class->SetText(strFmt("Class"), 0);
		auto Cape = (SText*)m_pControlContainer->FindControl(1919406);
		Cape->SetText(strFmt("Cape"), 0);
		...
		Page->SetText(strFmt("%d", g_pClientInfo->cConfigR.RankingPage), 0);
		Pos1->SetText(strFmt("1. %s", g_pClientInfo->Ranking.RankName[0]), 0);
		Pos2->SetText(strFmt("2. %s", g_pClientInfo->Ranking.RankName[1]), 0);
		Pos3->SetText(strFmt("3. %s", g_pClientInfo->Ranking.RankName[2]), 0);
		Pos4->SetText(strFmt("4. %s", g_pClientInfo->Ranking.RankName[3]), 0);
		Pos5->SetText(strFmt("5. %s", g_pClientInfo->Ranking.RankName[4]), 0);
		Pos6->SetText(strFmt("6. %s", g_pClientInfo->Ranking.RankName[5]), 0);
		Pos7->SetText(strFmt("7. %s", g_pClientInfo->Ranking.RankName[6]), 0);
		Pos8->SetText(strFmt("8. %s", g_pClientInfo->Ranking.RankName[7]), 0);
		Pos9->SetText(strFmt("9. %s", g_pClientInfo->Ranking.RankName[8]), 0);
		Pos10->SetText(strFmt("10. %s", g_pClientInfo->Ranking.RankName[9]), 0);

		lPos1->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[0]), 0);
		lPos2->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[1]), 0);
		lPos3->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[2]), 0);
		lPos4->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[3]), 0);
		lPos5->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[4]), 0);
		lPos6->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[5]), 0);
		lPos7->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[6]), 0);
		lPos8->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[7]), 0);
		lPos9->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[8]), 0);
		lPos10->SetText(strFmt("%d", g_pClientInfo->Ranking.RankLevel[9]), 0);
		...
		for (int i = 0; i < 10; i++) {
			...
			if (g_pClientInfo->Ranking.Kingdom[i] == 7 || g_pClientInfo->Ranking.Kingdom[i] == 8) {
				cape->sIndex = g_pClientInfo->Ranking.Kingdom[i] + 5800;
			}
			...
			classe->sIndex = g_pClientInfo->Ranking.RankClasse[i] + 5800;
			...
			if (g_pClientInfo->Ranking.RankEvolution[i] == 1)
				evolu->sIndex = 5804;
			if (g_pClientInfo->Ranking.RankEvolution[i] == 2)
				evolu->sIndex = 5805;
			if (g_pClientInfo->Ranking.RankEvolution[i] >= 3)
				evolu->sIndex = 5806;
	...
	if (p->State == 1) {
		...
		lPos1->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[0]), 0);
		lPos2->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[1]), 0);
		lPos3->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[2]), 0);
		lPos4->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[3]), 0);
		lPos5->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[4]), 0);
		lPos6->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[5]), 0);
		lPos7->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[6]), 0);
		lPos8->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[7]), 0);
		lPos9->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[8]), 0);
		lPos10->SetText(strFmt("%d", g_pClientInfo->Ranking.PvP[9]), 0);
...
int TMFieldScene::OnPacketUpdateKeys(MSG_STANDARD* pStd)
{
	...
	if (g_pClientInfo->Info.ReconnectDone) {
		g_GameAuto_mountValue = r_AutoMacroMP;
		g_GameAuto_hpValue = r_AutoMacroHP;
		g_GameAuto = r_AutoMacro;
		m_AutoHpMp = r_AutoMacroHPMP;
		m_AutoPostionUse = r_AutoMacroMove;
		SetAutoSkillNum(r_AutoMacroSkill);
		...
		else if (m_AutoPostionUse == 1)
		{
			...
			m_AutoStartPointX = r_AutoMacroX;
			m_AutoStartPointY = r_AutoMacroY;
...
int TMFieldScene::OnPacketUpdateJoias(MSG_STANDARD* pStd)
{
	...
	for (int x = 0; x < 10; x++) {
		for (int z = 0; z < 4; z++)
			memcpy(&g_pClientInfo->TrajesMount.Outros[z][x], &p->Outros[z][x], sizeof(STRUCT_ITEM));
	}
...
const char Mob_ClassMaster[6][13] =
{
	"Null",
	"Arch",
	"Mortal",
...
int TMFieldScene::OnPacketSendInfoPlayer(MSG_STANDARD* pStd) {

	auto PainelInfoPlay = (SGridControl*)m_pControlContainer->FindControl(456654);
	PainelInfoPlay->SetVisible(1);
	...
	auto p = reinterpret_cast<MSG_RequestInfoPlay*>(pStd);
	...
	auto InfoName = (SText*)m_pControlContainer->FindControl(456604);
	InfoName->SetText(strFmt("%s", p->Nick), 0);

	auto InfoNIVEL = (SText*)m_pControlContainer->FindControl(456609);
	InfoNIVEL->SetText(strFmt("%d", p->Level), 0);

	auto InfoEVO = (SText*)m_pControlContainer->FindControl(456605);
	InfoEVO->SetText(strFmt("%s", Mob_ClassMaster[p->Evolution]), 0);

	auto InfoClass = (SText*)m_pControlContainer->FindControl(456606);
	InfoClass->SetText(strFmt("%s", Mob_Class[p->Class]), 0);

	auto InfoGuild = (SText*)m_pControlContainer->FindControl(456607);

	if (strcmp(p->GuildName, "") != 0)
		InfoGuild->SetText(strFmt("%s", p->GuildName), 0);

	for (int slot = 1; slot < 18; slot++) {

		if (p->Equip[slot].sIndex == 0)
			continue;

		auto item = new STRUCT_ITEM;
		memset(item, 0, sizeof(STRUCT_ITEM));
		memcpy(item, &p->Equip[slot], sizeof(STRUCT_ITEM));

		auto EquipGrid = (SGridControl*)m_pControlContainer->FindControl(21000 + slot);
		EquipGrid->m_eGridType = TMEGRIDTYPE::GRID_SKILLB;
		EquipGrid->Empty();
		auto pItem = new SGridControlItem(0, item, 0.0f, 0.0f);
		EquipGrid->AddItem(pItem, 0, 0);
	}
	return 1;
}
int TMFieldScene::OnPacketUpdateDonate(MSG_STANDARD* pStd)
{
	...
	DonateInv->SetText(strFmt("Donate: %d", g_pClientInfo->Info.Cash), 0);
	DonateInv->SetTextColor(TNColor::GreenYellow);

	PixTx4->SetText(strFmt("Donate: %d", g_pClientInfo->Info.Cash), 0);
	PixTx4->SetTextColor(TNColor::CornBlue);

	auto PixTx3 = (SText*)m_pControlContainer->FindControl(188516);
	PixTx3->SetText(strFmt("Your Key: %s", g_pClientInfo->Info.Pix), 0);
	PixTx3->SetTextColor(TNColor::CornBlue);
...
int TMFieldScene::OnPacketUpdateDropList(MSG_STANDARD* pStd)
{
	...
	for (int z = 0; z < 64; z++)
		g_pClientInfo->DropList.Item[z] = 0;
...
int TMFieldScene::OnPacketAddParty(MSG_AddParty* pStd)
{
	...
	for (int i = 0; i < pPartyList->m_nNumItem; ++i)
	{
		...
		if (i == 0)
		{
			auto pHuman = (TMHuman*)g_pObjectManager->GetHumanByID(pPartyItem->m_dwCharID);
			if (pHuman) {
				sprintf(Reconnect_LeaderName, pHuman->m_szName);
				Reconnect_Conn_id_leader = pPartyItem->m_dwCharID;
			}
		}
		...
		if ((unsigned char)m_pMyHuman->m_stAffect[i].Type <= 99)
		{
			...
			if (m_pAffectIcon[i] && i && (unsigned char)m_pMyHuman->m_stAffect[i].Type > 0 || !i && nTime > 0)
			{
				...
				if (m_pAffectIcon[i]->m_bOver == 1)
				{
					...
					{
						if (m_pMyHuman->m_stAffect[i].Type == 51) {
							sprintf(strAffectString, "Revigorante : Montaria Imortal");
						}
						else {
...
DWORD WINAPI Guildmark_Download(void* pArg)
{
	...
	strcpy(strURL, "https://portalrogue.com/guilds/"); //Guild Mark Link
