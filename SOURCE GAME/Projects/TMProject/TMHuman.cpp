...
int TMHuman::OnPacketUpdateEquip(MSG_STANDARD* pStd)
{
    ...
    if (pEquip->sEquip[14] & 0xFFF && (pEquip->sEquip[14] & 0xFFF) == 3998){
       SetMountCostume(3998);
    }
...
int TMHuman::OnPacketUpdateAffect(MSG_STANDARD* pStd)
{
    ...
    if (pFScene && g_pCurrentScene->m_pMyHuman == this)
    {
        ...
        for (int i = 0; i < 32; ++i)
        {
            ...
            {
                if (pUpdateAffect->Affect[i].Level & 0x1)
                    m_DilpunchJewel = 1;
                if (pUpdateAffect->Affect[i].Level & 0x2)
                    m_MoonlightJewel = 1;
                if (pUpdateAffect->Affect[i].Level & 0x4)
                    m_JewelGlasses = 1;
                if (pUpdateAffect->Affect[i].Level & 0x8)
                    m_BloodJewel = 1;
                if (pUpdateAffect->Affect[i].Level & 0x10)
                    m_RedJewel = 1;
            }
        }
...
int TMHuman::OnPacketUpdateEtc(MSG_STANDARD* pStd)
{
    ...
    if (g_pCurrentScene->m_pMyHuman == this)
    {
        ...
        static_cast<TMFieldScene*>(g_pCurrentScene)->UpdateSkillBelt();
    ...
    if (!m_dwDelayDel)
    {
        ...
        if(pDest == nullptr)
        {
            ...
            if (m_pNickNameLabel)
            {
                ...
                else if (!strcmp(m_szNickName, "ROXO"))
                    m_pNickNameLabel->SetTextColor(TNColor::DeepPink);
                else if (!strcmp(m_szNickName, "LARANJA"))
                    m_pNickNameLabel->SetTextColor(TNColor::DarkOrange);
                else if (!strcmp(m_szNickName, "VERDE"))
                    m_pNickNameLabel->SetTextColor(TNColor::Speak);
                else if (!strcmp(m_szNickName, "AZUL"))
                    m_pNickNameLabel->SetTextColor(TNColor::CornBlue);
...
void TMHuman::SetAnimation(ECHAR_MOTION eMotion, int nLoop)
{
    if (!m_dwDelayDel && eMotion != ECHAR_MOTION::ECMOTION_NONE && m_pSkinMesh)
    {
        ...
        /*if (m_nSkinMeshType == 31 && (int)eMotion >= 4 && (int)eMotion <= 5)
            *(int*)eMotion += 3;*/
        if (m_nSkinMeshType == 31 && (int)eMotion == 4)
            eMotion = ECHAR_MOTION::ECMOTION_ATTACK04;
        if (m_nSkinMeshType == 31 && (int)eMotion == 5)
            eMotion = ECHAR_MOTION::ECMOTION_ATTACK05;
        if (m_nSkinMeshType == 31 && (int)eMotion == 6)
            eMotion = ECHAR_MOTION::ECMOTION_ATTACK06;
    ...
    if (m_pNameLabel)
    {
        ...
        if (pFScene->m_pMouseOverHuman != this
            && (m_nClass != 1 && m_nClass != 2 && m_nClass != 4 && m_nClass != 8 && m_nClass != 26 && 
                ...
                ((m_dwID < 0 || m_dwID >= 1000) && m_dwID < 25000)) &&
            !IsMerchant() && 
            m_bParty != 1 && 
            (int)m_usGuild <= 0 && 
            ((m_dwID < 0 || m_dwID >= 1000 && m_dwID < 25000)) &&
...
void TMHuman::Die()
{
    ...
    if (TMHuman::m_dwID > 0 && TMHuman::m_dwID < 1000)//fix personagem some após 10 segundos que morre
    {
        SetAnimation(ECHAR_MOTION::ECMOTION_DEAD, 1);
    }
    else
...
void  TMHuman::SetMountCostume(unsigned int  index)
{
    ...
    if (index >= 11 && index <= 200 || index == 3998 && (!(m_stScore.Reserved & 2)))
    {
