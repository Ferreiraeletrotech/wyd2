...
int TMSelectCharScene::OnControlEvent(unsigned int idwControlID, unsigned int idwEvent)
{
	...
	if (idwControlID == 66433)
	{
		if (!g_AccountLock)
		{
			...
			strcpy(Numerica, msLock.ItemPassWord);
	...
	if (idwControlID == 4612 && dwServerTime - m_dwLastClickLoginBtnTime > 2000)
	{
		...
		nPersonagem = g_pObjectManager->m_cCharacterSlot;
		if (nSlot < 0 || nSlot >= 4)
		{
		...
		else if (m_pHuman[nSlot] && pSelChar->MobName[nSlot][0])
		{
			...
			g_pClientInfo->Info.LoginDone = 1;
...
{
	if (ReconnectTimer == 5 && ReconnectState == 2) {
		Reconnecting_Numerica();
		ReconnectTimer = 0;
		ReconnectState = 3;
		ReqAlias(29);
	}
	if (ReconnectTimer == 5 && ReconnectState == 3) {
		Reconnecting_Char();
		ReconnectTimer = 0;
		ReconnectState = 4;
		//Reconnect = false;
...
void TMSelectCharScene::VisibleSelectCreate(int bSelect)
{
	...
	if (bSelect != 1)
	{
		...
		pCreWin->SetPos(RenderDevice::r_fWidthRatio * 514.0f, RenderDevice::r_fHeightRatio * 35.0f);
	...
	else
	{
		...
		pSelChar->SetPos(RenderDevice::r_fWidthRatio * 514.0f, RenderDevice::r_fHeightRatio * 35.0f);
