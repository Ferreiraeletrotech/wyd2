...
TMScene::TMScene() : TreeNode(0)
{
	...
	//msgpanel
	m_pMessagePanel = new SMessagePanel("Message Panel", 0.0f, 80.0f, 480.0f, 24.0f, 2000);
	...
	m_pAlphaNative = new SText(-2, "BR", 0xFFFFFFDD, 0.0f, fHeight, 28.0f, 16.0f, 1, 0xAA000077, 1, 0);
...
int TMScene::ReadRCBin(char* szBinFileName)
{
	...
	while (fread(&nControlType, 4, 1, fpBinary))
	{
		...
		case CONTROL_TYPE::CTRL_TYPE_PANEL:
		{
			...
			{
				m_pControlContainer->AddItem(pPanel);
			}
		...
		case CONTROL_TYPE::CTRL_TYPE_GRID:
		{
			...
			if (binGridData.nParentID)
			{
				...
				if (pParent)
					pParent->AddChild(pGrid);
			}
			else
			{
				m_pControlContainer->AddItem(pGrid);
			}
		}
		break;
		case CONTROL_TYPE::CTRL_TYPE_3DOBJ:
		{
			...
			if (bin3DObjData.nParentID)
			{
				...
				if (pParent)
					pParent->AddChild(p3DObj);
			}
			else
			{
				m_pControlContainer->AddItem(p3DObj);
			}
		}
		break;
		case CONTROL_TYPE::CTRL_TYPE_BUTTON:
		{
			...
			{
				m_pControlContainer->AddItem(pButton);
			}
		...
		case CONTROL_TYPE::CTRL_TYPE_TEXT:
		{
			...
			{
				m_pControlContainer->AddItem(pText);
			}
		}
		break;
