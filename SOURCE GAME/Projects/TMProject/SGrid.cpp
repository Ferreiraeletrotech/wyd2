...
std::string quickAddThousandSeparators2(std::string value, char thousandSep = ',')
{
...
int SGridControl::OnMouseEvent(unsigned int dwFlags, unsigned int wParam, int nX, int nY)
{
	...
	if (dwFlags == 513)
	{
		...
		if (g_pCursor->GetStyle() == ECursorStyle::TMC_CURSOR_HAND && bPtInRect)
		{
			...
			if (bClick && m_eGridType == TMEGRIDTYPE::GRID_SKILLB) {
				int GridID = SGridControl::GetControlID();


				 
				if (GridID == 179600 || GridID == 279600)
				{

					auto pItem = GetItem(nCellX, nCellY);
					std::cout << "\n pItem " << pItem;
					if (pItem) {
						int Index = pItem->m_pItem->sIndex;
				...
				//New Composer
				if (GridID >= 181924 && GridID <= 181939) {
					...
					if (pItem) {
						...
						if (Index != 0) {
							g_pClientInfo->Info.Composition = GridID - 181924;

							MSG_ReqNPC stReqNPC{};

							stReqNPC.Type = _MSG_ReqNPC;
							stReqNPC.ID = pScene->m_pMyHuman->m_dwID;
							stReqNPC.State = 1;
	...
	else if (dwFlags == 513 && g_pEventTranslator->m_bShift)
	{
		...
		if (pItem->m_pItem->sIndex == 412)
			...
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 3140)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 3336)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 4010)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 4011)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 3326)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 4146)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 787)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 4097)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 3182)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 777)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 4140)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 4144)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 3173)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 4046)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 1774)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 415)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 697)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 677)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 678)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 5643)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex == 5603)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex >= 5824 && pItem->m_pItem->sIndex <= 5837)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex >= 5776 && pItem->m_pItem->sIndex <= 5783)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex >= 5110 && pItem->m_pItem->sIndex <= 5133)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex >= 5844 && pItem->m_pItem->sIndex <= 5855)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex >= 4016 && pItem->m_pItem->sIndex <= 4020)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex >= 4026 && pItem->m_pItem->sIndex <= 4029)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex >= 3027 && pItem->m_pItem->sIndex <= 3030)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex >= 2441 && pItem->m_pItem->sIndex <= 2444)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex >= 4051 && pItem->m_pItem->sIndex <= 4055)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex >= 2390 && pItem->m_pItem->sIndex <= 2419)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex >= 3200 && pItem->m_pItem->sIndex <= 3220)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex >= 3324 && pItem->m_pItem->sIndex <= 3327)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex >= 5876 && pItem->m_pItem->sIndex <= 5878)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex >= 5624 && pItem->m_pItem->sIndex <= 5626)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex >= 5752 && pItem->m_pItem->sIndex <= 6001)
			itemcheck = true;
		else if (pItem->m_pItem->sIndex >= 5972 && pItem->m_pItem->sIndex <= 6500)
			itemcheck = true;
		...
		if (pItem)
		{
			if (pItem->m_GCObj.dwColor != 0xFFFF0000)
			{
				...
				if (sDestPos == -1)
				{
					...
					//Fazer troca do colar e cinto segurando control
					//colar
					if (pItem->m_pItem->sIndex >= 4483 && pItem->m_pItem->sIndex <= 4497)
						stSwapItem.SourPos = 17;

					//cinto
					if (pItem->m_pItem->sIndex >= 4468 && pItem->m_pItem->sIndex <= 4482)
						stSwapItem.SourPos = 16;
...
int SGridControl::MouseOver(int nCellX, int nCellY, int bPtInRect)
{
	...
	else if (pItem->m_pItem->sIndex >= 10000 && pItem->m_pItem->sIndex < 11500)
	{
		...
		for (int i = 0; i < 9; ++i)
		{
			...
			{
				pFScene->m_pParamText[i]->SetTextColor(0xFFFFFFFF);
	...
	if (sIndex == 5722) {
		...
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[3]->SetText(strFmt("Fragmentos"), 0);
		pFScene->m_pParamText[7]->SetTextColor(Laranja);
		pFScene->m_pParamText[7]->SetText(strFmt("Libera o Drop dos Itens Citados"), 0);
	}
	if (sIndex == 5723) {
	...
	if (m_eGridType != TMEGRIDTYPE::GRID_SHOP)
	{
		if (m_eGridType == TMEGRIDTYPE::GRID_SELL)
		{
			...
			for (int i = 0; i < 3; i++) {
				if (pItem->m_pItem->stEffect[i].cEffect == 61) {
					Qnt = pItem->m_pItem->stEffect[i].cValue;
				}
			}

			std::string sCoin;
			std::string sCoinF;
			sCoin = strFmt("%d", (nPrice * Qnt));
			sCoinF = quickAddThousandSeparators2(sCoin);
			char Porcentagem[2] = "%";

			//PREÇO DOS ITENS DE VENDA
			sprintf(szText, "Preço de Venda: %s (Tax: 15%s)", sCoinF.c_str(), Porcentagem);
		...
		else
		{
			...
			if (!modded)
			{
				...
				else
				{
					...
					std::string Coin;
					std::string CoinF;
					Coin = strFmt("%d", ((nItemPrice * Qnt) + (int)taxPrice));
					CoinF = quickAddThousandSeparators2(Coin);

					//mostra preço dos itens pra comprar
					sprintf(szText, "Preço: %s", CoinF.c_str());
					sprintf(szText, "%s (%s:%d%%)", szText, g_pMessageStringTable[146], 15);
				}
			}
...
int SGridControl::Check_ItemRightClick(int nType, int nItemSIndex)
{
	...
	switch (nItemSIndex) // RIGHTCLICKITEM
	{
	case 4146:
		return 1;
	case 4147:
	...
	if (nItemSIndex >= 4911 && nItemSIndex <= 4915)
		return 1;
	if (nItemSIndex >= 5844 && nItemSIndex <= 5849)
		return 1;
	if (nItemSIndex >= 5856 && nItemSIndex <= 5865)
		return 1;
