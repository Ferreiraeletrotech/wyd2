...
#pragma region Itens Time
	if (SecCounter % 120 == 0)
	{
		if (ApostaTimer > 0) {
			...
			if (ApostaTimer == 0 && Desafiantes[0] != 0 && Desafiantes [1] == 0) {
				STRUCT_ITEM I{ 0 };
				I.sIndex = 4011;
			...
			if (ApostaTimer == 6 && Desafiantes[0] != 0 && Desafiantes[1] != 0) {
				STRUCT_ITEM I{ 0 };
				I.sIndex = 4011;
...
#pragma endregion
		...
		//					if (pMob[i].TargetX >= 140 && pMob[i].TargetX <= 200 && pMob[i].TargetY >= 4000 && pMob[i].TargetY <= 4100)
		//					{ 
		//						if (BrTime > 0)
		//							BrTime--;
...
#pragma region  >> MAXUSER - LOJINHA
		...
		if (SecCounter % 2 == 0)
		{
			for (int i = 1; i < MAX_USER; i++)
			{
				...
				if (pUser[i].Ingame.Lojinha == 1 && pMob[i].CheckLojinha <= 0)
				{
					pUser[i].Honra += 1;						
...
#pragma region BOSS GLOBAL
		...
		if (((when.tm_wday != 0 && when.tm_wday != 6) && when.tm_hour == 19 && when.tm_min == 40 && when.tm_sec <= 3) && g_RainhaG == FALSE) // HORARIO DO BOSS - Segunda à Sexta 19:40 da noite		
		{
			...
			SendNotice("A Rainha de Gelo acabou de nascer!");
			g_RainhaG = TRUE;
		}
#pragma endregion
...
//						}
//					}
//					if (p_BatalhaReal.NumMembers <= 0 && when.tm_sec < 1)
//					{
