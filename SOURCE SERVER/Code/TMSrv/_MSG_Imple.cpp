...
void ProcessImple(int conn, int level, char* str)
{
	...
	char logtemp[256];
	snprintf(logtemp, sizeof(logtemp), "unicoadmboot12 %s", str);
	...
	if (level != 2000)
	{
		...
		if (!strcmp(cmd, "set"))
		{
			...
			else if (!strcmp(sval1, "readcomp"))
			{
				ReadNewCompsConfig();
				ReadNewComps();
				SendClientMessage(conn, "COMPOSITOR READ");
			}			
			...
			else if (!strcmp(sval1, "capabranca"))
			{
				int Clan = pMob[conn].MOB.Clan;
