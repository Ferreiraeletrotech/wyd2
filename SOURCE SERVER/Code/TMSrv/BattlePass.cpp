...
void ReloadBattlePassItens() {
	...
	sprintf(hQuery, "SELECT * FROM `battlepassitens` LIMIT 50");
	MYSQL_ROW row;
...
void ReloadBattlePassMissions() {
	...
	sprintf(hQuery, "SELECT * FROM `bpassmissions` LIMIT 50");
	MYSQL_ROW row;
	MYSQL* wSQL = pc.wStart();
	MYSQL_RES* result = pc.wRes(wSQL, hQuery);
...
void BattlePassReward(int conn) {
	...
	if (BattlePassPremium && pUser[conn].Keys[46] == 0) {
		SendClientMessage(conn, "Você Precisa adquirir o Passe de Batalha");
		return;
	}
