...
#pragma region Poeira de Fada
	if (Vol == 7)//Poeira de Fada
	{
		...
		int maxlevel = pMob[conn].extra.ClassMaster == MORTAL ? MAX_LEVEL : MAX_CLEVEL;

		if (pMob[conn].extra.ClassMaster >= CELESTIAL || pMob[conn].extra.ClassMaster == ARCH)
		{
			SendClientMessage(conn, "Desculpe. Item disponível apenas para Mortais .");
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma region Gema Estelar
	...
	CanSave:
		...
		pUser[conn].Keys[37] = pMob[conn].TargetX;
		pUser[conn].Keys[38] = pMob[conn].TargetY;
...
#pragma region Portal
	if (Vol == 13)
	{
		pMob[conn].MOB.SPX = pUser[conn].Keys[37];
		pMob[conn].MOB.SPY = pUser[conn].Keys[38];
...
#pragma region Pergaminho da agua N
	if (Vol >= 131 && Vol <= 140)
	{
		...
		if ((pMob[conn].extra.ClassMaster == CELESTIAL || pMob[conn].extra.ClassMaster == ARCH || pMob[conn].extra.ClassMaster == CELESTIALCS || pMob[conn].extra.ClassMaster == SCELESTIAL) && pMob[conn].MOB.CurrentScore.Level > -1)
		{
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
...
#pragma region >> Baú_de_Secretas
	if (item->sIndex == 5850)
	{
		// Nova lista com 4 opções de itens
		struct ItemOption {
			int sIndex;
			int effect1;
			int value1;
		...
		int randIndex = rand() % 4; // Sorteia de 0 a 3

		STRUCT_ITEM Item;
		memset(&Item, 0, sizeof(STRUCT_ITEM));

		// Define o item sorteado
		Item.sIndex = options[randIndex].sIndex;
		Item.stEffect[0].cEffect = options[randIndex].effect1;
		Item.stEffect[0].cValue = options[randIndex].value1;
		Item.stEffect[1].cEffect = options[randIndex].effect2;
		Item.stEffect[1].cValue = options[randIndex].value2;
...
#pragma endregion
#pragma region Caixa do iniciante
	if (item->sIndex == 5971)
	{
		// Estrutura para definir armas e seus efeitos
		struct WeaponConfig {
			int sIndex;  // ID da arma
			int effect1; // Primeiro efeito
			int value1;  // Valor do primeiro efeito
			int effect2; // Segundo efeito
			int value2;  // Valor do segundo efeito
		};

		// Lista de 5 armas fixas com efeitos personalizados
		WeaponConfig fixedWeapons[] = {
			{3314, 61, 10, 0, 0},  // Frango
			{4144, 61, 10, 0, 0},  // BauXP
			{4019, 61, 120, 0, 0}, // ClasseD
			{4129, 0, 0, 0, 0}, // Arma 4
			{3140, 61, 120, 0, 0}  // Arma 5
		};

		for (int i = 0; i < 5; i++) // Loop para entregar as 5 armas
		{
			STRUCT_ITEM Item;
			memset(&Item, 0, sizeof(STRUCT_ITEM));

			Item.sIndex = fixedWeapons[i].sIndex;
			Item.stEffect[0].cEffect = fixedWeapons[i].effect1;
			Item.stEffect[0].cValue = fixedWeapons[i].value1;
			Item.stEffect[1].cEffect = fixedWeapons[i].effect2;
			Item.stEffect[1].cValue = fixedWeapons[i].value2;

			PutItem(conn, &Item); // Entrega a arma ao jogador
		}
		...
		SendCarry(conn);
		SendClientMessage(conn, "Você recebeu o kit migração!");

		return;
	}
#pragma endregion
#pragma region Bolao da Guild
	if (item->sIndex == 5989)
	{
		// Estrutura para definir armas e seus efeitos
		struct WeaponConfig {
		...
		// Lista de 5 armas fixas com efeitos personalizados
		WeaponConfig fixedWeapons[] = {
			{3314, 61, 120, 0, 0},  // Frango
			{4144, 61, 120, 0, 0},  // BauXP
			{3381, 0, 0, 0, 0}, // ClasseD
			{4129, 0, 0, 0, 0}, // Esfera M
			{3172, 61, 15, 0, 0}  // Carta Duelo
		};
		...
		SendCarry(conn);
		SendClientMessage(conn, "Você recebeu o kit Bolao GUild, Obrigado pelo apoio!");
...
#pragma endregion
#pragma region Caixa de Joias
	if (item->sIndex == 5810)
	{
		STRUCT_ITEM Item;
		memset(&Item, 0, sizeof(STRUCT_ITEM));

		int Joias[] = { 2442, 2441, 2443, 2444 };

		int _rand = rand() % (sizeof(Joias) / 4);
		Item.sIndex = Joias[_rand];
		Item.stEffect[0].cEffect = 0;
		Item.stEffect[0].cValue = 0;
...
#pragma endregion
#pragma region Caixa de Brincos
	if (item->sIndex == 5789)
	{
		STRUCT_ITEM Item;
		memset(&Item, 0, sizeof(STRUCT_ITEM));

		int Brincos[] = { 591, 592, 593, 594, 595 };

		int _rand = rand() % (sizeof(Brincos) / 4);
		Item.sIndex = Brincos[_rand];
		Item.stEffect[0].cEffect = 43;
		Item.stEffect[0].cValue = 230;
...
#pragma region >> BAU CINTO
	if (item->sIndex == 5856)
	{
		...
		if (rand_ <= 25)
		{
			Item.sIndex = 4479;
			Item.stEffect[0].cEffect = 43; //efeito REF
			Item.stEffect[0].cValue = 9; //valor
		...
		else if (rand_ <= 50)
		{
			...
			Item.stEffect[0].cEffect = 43; //efeito REF
			Item.stEffect[0].cValue = 9; //valor
			Item.stEffect[1].cEffect = 0; //efeito deff
			Item.stEffect[1].cValue = 0; //valor
		...
		else if (rand_ <= 75)
		{
			...
			Item.stEffect[0].cEffect = 43; //efeito REF
			Item.stEffect[0].cValue = 9; //valor
			Item.stEffect[1].cEffect = 0; //efeito skill
			Item.stEffect[1].cValue = 0; //valor
...
#pragma region >> Bau Arcano 5844
	...
	if (item->sIndex == 5844)
	{
		...
		if (rand_ <= 10)
		{
			Item.sIndex = 563; //Amuleto_Arcano
			Item.stEffect[0].cEffect = 2; //efeito REF
			Item.stEffect[0].cValue = 45; //valor
		}
		else if (rand_ <= 20)
		{
			Item.sIndex = 564; //Amuleto_Arcano
			Item.stEffect[0].cEffect = 60; //efeito REF
