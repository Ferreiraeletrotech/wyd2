...
#pragma endregion
	...
	case APOSTA:
	{
		if (Desafiantes[0] != 0 && Desafiantes[1] != 0) {
			SendSay(conn, "Uma aposta está em andamento");
			return;
		}
		if (Desafiantes[0] != 0 && Desafiantes[1] == 0) {
			if (RemoveItem(conn, 4011, 1) == true) {
				Desafiantes[1] = conn;
				DoTeleport(conn, 1083, 1733);
				DoTeleport(Desafiantes[0], 1083, 1727);
				SendNotice(strFmt("O duelo entre %s e %s começou!", pMob[Desafiantes[0]].MOB.MobName, pMob[conn].MOB.MobName));
				ApostaTimer = 11;
			}
			else {
				SendSay(conn, "Você precisa de [1] Barra de 1 Bi");
			}
			return;
		}
		if (Desafiantes[0] == 0) {
			if (RemoveItem(conn, 4011, 1) == true) {
				Desafiantes[0] = conn;	
				ApostaTimer = 5;
				SendNotice(strFmt("O Jogador %s deu inicio à aposta PvP em Noatum!", pMob[conn].MOB.MobName));
				SendClientMessage(conn, "!O desafio será cancelado e você receberá seu dinheiro de volta");
				SendClientMessage(conn, "!Em aproximadamente 5 minutos se não houver desafiante");
			}
			else {
				SendSay(conn, "Você precisa de [1] Barra de 1 Bi");
			}
