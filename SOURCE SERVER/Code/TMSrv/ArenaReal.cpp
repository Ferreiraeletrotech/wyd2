...
void ArenaRealTimer() {
	...
	ArenaReal.EvoMin = 0; //Evolução minima pra entrar. 0 - mortal, 1 - arch, 2 - cele
	ArenaReal.LevelMin = 351; //level mínimo pra entrar
...
void StartArenaReal() {
	...
	// Verifica se os participantes cadastrados estão online. Se não estiverem, são retirados da party
	// Conta quantos participantes estão online em cada equipe
	...
	//Se não houver pelo menos 1 participante online em cada equipe, não inicia
	if (Count[0] < 1 || Count[1] < 1 || Count[2] < 1 || Count[3] < 1) {
		SendNotice("Não há participantes suficiente para ocorrer a Arena Real");
		return;
	}
	...
	for (int z = 0; z < 4; z++) {
		for (int x = 0; x < LIMITE_PARTIC; x++) {
			...
			if (Leaders[z] == 0) {
				...
				if (pUser[conn].Keys[39] == 0) {
					for (int i = 1; i < 200; i++) {
						if (Parties[i].Index == 0) {
							...
							strncpy(Parties[i].Nick, pMob[conn].MOB.MobName, 16);
				...
				strncpy(sm_cap.MobName, pMob[idx].MOB.MobName, sizeof(sm_cap.MobName));
	...
	for (int p = 0; p < 4; p++) {
		for (int f = 0; f < LIMITE_PARTIC; f++) {
			...
			SendClientMessage(conn, "A Arena Real Começou!");
	...
	for (int c = 0; c < LIMITE_PARTIC; c++) {
		for (int k = 0; k < 4; k++) {
			...
			if (Venceu[k] == 0 && conn != 0) {
				...
				SendClientMessage(conn, "Obrigado por participar da Arena Real, Mais sorte na próxima!");
			...
			if (Venceu[k] == 1 && conn != 0) {
				...
				SendClientMessage(conn, "Parabéns por vencer na Arena Real!");
				pUser[conn].Keys[30] = 0;
				pUser[conn].Keys[31] = 0;
...
void RegistroArenaReal(int conn) {
	...
	if (ArenaReal.State != 1) {
		SendClientMessage(conn, "O registro da Arena Real ainda não foi liberado!");
		return;
	}
	...
	if (Evo < ArenaReal.EvoMin) {
		SendClientMessage(conn, "Sua evolução não é permitida");
		return;
	}

	if (pMob[conn].MOB.CurrentScore.Level < (ArenaReal.LevelMin - 1)) {
		SendClientMessage(conn, strFmt("Level mínimo: %d", ArenaReal.LevelMin));
		return;
	}

	if (pUser[conn].Keys[30] == 1) {
		SendClientMessage(conn, "Você já está registrado!");
		return;
	}
		...
		if (ArenaReal.Jogadores.Player[Team][x] == 0) {
			...
			if (ArenaReal.GoldRegistro > 0) {
				if (pMob[conn].MOB.Coin < ArenaReal.GoldRegistro) {
					SendClientMessage(conn, strFmt("Você precisa de [%d] Gold pra se registrar!", ArenaReal.GoldRegistro));
					return;
				}
			}
			if (ArenaReal.ItemRegistro.sIndex != 0) {
				if (RemoveItem(conn, ArenaReal.ItemRegistro.sIndex, 1) == false) {
					SendClientMessage(conn, strFmt("Você precisa de um [%s] pra se registrar!", g_pItemList[ArenaReal.ItemRegistro.sIndex].Name));
					return;
				}
			}
	...
	if (Registrado == 0) {
		SendClientMessage(conn, "Você foi registrado na equipe [Roxa]");
	}
	if (Registrado == 1) {
		SendClientMessage(conn, "Você foi registrado na equipe [Laranja]");
	}
	if (Registrado == 2) {
		SendClientMessage(conn, "Você foi registrado na equipe [Verde]");
	}
	if (Registrado == 3) {
		SendClientMessage(conn, "Você foi registrado na equipe [Azul]");
	}
	if (Registrado == -1) {
		SendClientMessage(conn, "Não havia vagas para registro");
		return;
	}
