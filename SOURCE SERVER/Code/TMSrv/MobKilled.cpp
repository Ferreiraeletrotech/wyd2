...
void MobKilled(int target, int conn, int PosX, int PosY)
{
	...
	if ((conn > 0 && conn < MAX_USER) && (target > 0 && target < MAX_USER)) {
		if ((conn == Desafiantes[0] || conn == Desafiantes[1]) && (target == Desafiantes[0] || target == Desafiantes[1])) {
			Desafiantes[0] = 0;
			Desafiantes[1] = 0;
						
			DoTeleport(target, 2132 + rand() % 5, 2147 + rand() % 5);	
			pMob[target].MOB.CurrentScore.Hp = pMob[target].MOB.CurrentScore.MaxHp;
			pUser[target].NumError = 0;
			...
			GetCreateMob(target, &CreateMob);
			GridMulticast(pMob[target].TargetX, pMob[target].TargetY, (MSG_STANDARD*)&CreateMob, 0);
						
			DoTeleport(conn, 2132 + rand() % 5, 2147 + rand() % 5);
			...
			STRUCT_ITEM I{ 0 };
			I.sIndex = 4011;
			I.stEffect[0].cEffect = 61;
			I.stEffect[0].cValue = 2;
			PutItem(conn, &I);

			SendClientMessage(target, "Infelizmente você perdeu...");
			SendClientMessage(conn, "!Recebeu [2] Barras de Bi");
			SendNotice(strFmt("O jogador %s venceu o duelo contra o jogador %s!", pMob[conn].MOB.MobName, pMob[target].MOB.MobName));
			return;
		}		
	}
		if (BrState >= 1)
	...
	#pragma region >> Func para quem morreu
			...
			//			
			//			bPassMission(conn, 0);
			//			STRUCT_ITEM presente = { 4026, 61, 2, 0 };
...
#pragma region Sombra Negra
						if (pMob[target].GenerateIndex == SOMBRANEGRA)
						{
							...
							if (PutItem(conn, &item)){
								SendClientMessage(conn, "Você recebeu [01][Barra_de_Prata(50Mi)]");
								
							}
							g_BSombraNegra = FALSE;
							bPassMission(conn, 23);
						}
#pragma endregion
...
#pragma region Verid Negro
						if (pMob[target].GenerateIndex == VERIDNEGRO)
						{
							...
							if (PutItem(conn, &item))
								SendClientMessage(conn, "Você recebeu [02][Barra_de_Prata(50Mi)]");
							g_BVerid = FALSE;
							bPassMission(conn, 24);
						}
#pragma endregion
#pragma region Verid Negro
...
#pragma region No lose At war
		if (conn < MAX_USER)
		{
			...
			//boss Rainha
			if (pMob[target].TargetX >= 3709 && pMob[target].TargetX <= 3778 && pMob[target].TargetY >= 2955 && pMob[target].TargetY <= 3012)
				AtWar = 1;
