...
#pragma region _MSG_DBCNFCharacterLogin
		case _MSG_DBCNFCharacterLogin:
		{
			...
			{
				CloseUser(conn);//for�a a saida da conn
				break;
			}
			...
			if (KefraLive == 0) {
				SendMsgExp(conn, "Kefra est� Vivo!", Orange, FALSE);
			}*/
			...
			//bool Reboot = false;
			//for (int i = 0; i < 16; i++) {
			//	if (pMob[conn].MOB.Equip[i].sIndex == 632 || pMob[conn].MOB.Equip[i].sIndex == 671 || pMob[conn].MOB.Equip[i].sIndex == 670) // ABS - ESPECTRAL - CONCENTRA�O
			//	{
			...
			//for (int i = 0; i < 64; i++) {
			//	if (pMob[conn].MOB.Carry[i].sIndex == 632 || pMob[conn].MOB.Carry[i].sIndex == 671 || pMob[conn].MOB.Carry[i].sIndex == 670) // ABS - ESPECTRAL - CONCENTRA�O
			//	{
			...
			//}
			//for (int i = 0; i < 128; i++) {
			//	if (pUser[conn].Cargo[i].sIndex == 632 || pUser[conn].Cargo[i].sIndex == 671 || pUser[conn].Cargo[i].sIndex == 670) // ABS - ESPECTRAL - CONCENTRA�O
			//	{
			...
			if (pMob[conn].MOB.MobName != 0)
			{
				sprintf(temp, "[%s] acabou de entrar!", pMob[conn].MOB.MobName);
				SendSpkMsg(conn, temp, TNColor::GoldenEscuro, true);
			}
			...
			pMob[conn].GetCurrentScore(conn);
			SendScore(conn);
			if (Parties[pUser[conn].Keys[39]].Index == 0) {
				pUser[conn].Keys[39] = 0;
			}

			if (pUser[conn].Reconnected == FALSE) {
				pUser[conn].Keys[32] = 0;
			}
			if (pUser[conn].Reconnected == TRUE) {
				SendReconnectParty(conn);

				int x = pUser[conn].Keys[37];
				int y = pUser[conn].Keys[38];
