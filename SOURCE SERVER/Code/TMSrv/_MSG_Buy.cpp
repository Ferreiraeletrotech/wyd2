...
void Exec_MSG_Buy(int conn, char* pMsg)
{
	...
	if (strcmp(pMob[TargetID].MOB.MobName, "Thorin") == 0) //npc pvp armia batalha real
	{
		...
		if (itemIndex == 5661) {
			RegistroArenaReal(conn);
		}
	}

	if (strcmp(pMob[TargetID].MOB.MobName, "Exchange") == 0)
	{
		STRUCT_ITEM Item{ 0 };

		const int Qnt = 17; //Colocar quantidade de trocas existentes

		int ItemSolicitado[Qnt][2] = {
			{ 419, 120}, //Item 1 solicitado << INDEX DO ITEM, QUANTIDADE >>
			{ 2398, 120}, //Item 3 solicitado << INDEX DO ITEM, QUANTIDADE >>
			{ 2403, 120}, //Item 4 solicitado << INDEX DO ITEM, QUANTIDADE >>
			{ 5811, 40}, //Item 5 solicitado << INDEX DO ITEM, QUANTIDADE >>
			{ 4479, 1}, //Item 6 solicitado << INDEX DO ITEM, QUANTIDADE >>
			{ 4480, 1}, //Item 7 solicitado << INDEX DO ITEM, QUANTIDADE >>
			{ 4481, 1}, //Item 8 solicitado << INDEX DO ITEM, QUANTIDADE >>
			{ 4482, 1}, //Item 9 solicitado << INDEX DO ITEM, QUANTIDADE >>
			{ 4478, 1}, //Item 10 solicitado << INDEX DO ITEM, QUANTIDADE >>
			{ 4493, 1}, //Item 11 solicitado << INDEX DO ITEM, QUANTIDADE >>
			{ 4494, 1}, //Item 12 solicitado << INDEX DO ITEM, QUANTIDADE >>
			{ 4495, 1}, //Item 13 solicitado << INDEX DO ITEM, QUANTIDADE >>
			{ 4496, 1}, //Item 14 solicitado << INDEX DO ITEM, QUANTIDADE >>
			{ 4497, 1}, //Item 15 solicitado << INDEX DO ITEM, QUANTIDADE >>
			{ 413, 7} //Item 16 solicitado << INDEX DO ITEM, QUANTIDADE >>
		};
		int ItemEntregue[Qnt][2] = {
			{ 413, 5}, //Item 1 Entregue << INDEX DO ITEM, QUANTIDADE >>
			{ 2404, 5}, //Item 3 Entregue << INDEX DO ITEM, QUANTIDADE >>
			{ 2399, 5}, //Item 4 Entregue << INDEX DO ITEM, QUANTIDADE >>
			{ 4141, 3}, //Item 5 Entregue << INDEX DO ITEM, QUANTIDADE >>
			{ 4479,}, //Item 6 Entregue << INDEX DO ITEM, QUANTIDADE >>
			{ 4480}, //Item 7 Entregue << INDEX DO ITEM, QUANTIDADE >>
			{ 4481}, //Item 8 Entregue << INDEX DO ITEM, QUANTIDADE >>
			{ 4482}, //Item 9 Entregue << INDEX DO ITEM, QUANTIDADE >>
			{ 4478}, //Item 10 Entregue << INDEX DO ITEM, QUANTIDADE >>
			{ 4493}, //Item 11 Entregue << INDEX DO ITEM, QUANTIDADE >>
			{ 4494}, //Item 12 Entregue << INDEX DO ITEM, QUANTIDADE >>
			{ 4495}, //Item 13 Entregue << INDEX DO ITEM, QUANTIDADE >>
			{ 4496}, //Item 14 Entregue << INDEX DO ITEM, QUANTIDADE >>
			{ 4497}, //Item 15 Entregue << INDEX DO ITEM, QUANTIDADE >>
			{ 5134, 1} //Item 16 Entregue << INDEX DO ITEM, QUANTIDADE >>
		};

		for (int x = 0; x < Qnt; x++) {
			if (itemIndex == ItemEntregue[x][0]) {

				if (pUser[conn].Lasclick != itemIndex) {
					SendClientMessage(conn, strFmt("Você precisa de %d %s. Deseja prosseguir?", ItemSolicitado[x][1], g_pItemList[ItemSolicitado[x][0]].Name));
					pUser[conn].Lasclick = itemIndex;
					return;
				}

				if (RemoveItem(conn, ItemSolicitado[x][0], ItemSolicitado[x][1]) == true) {
					Item.sIndex = ItemEntregue[x][0];
					Item.stEffect[0].cEffect = 61;
					Item.stEffect[0].cValue = ItemEntregue[x][1];
					if (PutItem(conn, &Item) == false) {
						ItemLog(pUser[conn].AccountName, pUser[conn].MacAddress, pUser[conn].IP, 
							strFmt("Exchange - Não houve espaço para entregar [%d]x[%d]", ItemEntregue[x][1], ItemEntregue[x][0]));
					}
				}
				else {
					SendClientMessage(conn, strFmt("Você precisa de %d %s, para efetuar essa troca",
						ItemSolicitado[x][1], g_pItemList[ItemSolicitado[x][0]].Name));
				}
				break;
			}
		}
