...
STRUCT_PARTIES Parties[200];

int Desafiantes[2] = { 0 , 0};
int ApostaTimer = 0;

int bPassMissionsPoints[50] = {};

STRUCT_ITEM BattlePassItens[50];
...
std::vector<STRUCT_RECENTCONNECTION> BlackList = std::vector<STRUCT_RECENTCONNECTION>();
...
CMob pMob[MAX_MOB + MAX_USER + 200];
...
#pragma endregion
	...
	{ 
		...
		AppendMenu(hSubMenu, MF_STRING, IDC_RELOADNEWCOMPOSITOR, "&Reload New Compositor");
...
#pragma endregion*/
	...
	if (Mode && Mode != USER_ACCEPT)
	{
		...
		else
		{
			...
			if (conn == Desafiantes[0]) {
				STRUCT_ITEM I{ 0 };
				I.sIndex = 4011;

				if (pUser[Desafiantes[1]].Mode == USER_PLAY) {
					PutItem(Desafiantes[1], &I);
					SendClientMessage(Desafiantes[1], "!Desafio de 1x1 cancelado!");
					SendClientMessage(Desafiantes[1], "!Você recebeu sua barrra de Bi de volta");
					DoTeleport(Desafiantes[1], 2100, 2100);

					PutItem(Desafiantes[1], &I);
					SendClientMessage(Desafiantes[1], "!Você venceu por W.O.");
				}
			...
			if (conn == Desafiantes[1]) {
				STRUCT_ITEM I{ 0 };
				I.sIndex = 4011;

				if (pUser[Desafiantes[0]].Mode == USER_PLAY) {
					PutItem(Desafiantes[0], &I);
					SendClientMessage(Desafiantes[0], "!Desafio de 1x1 cancelado!");
					SendClientMessage(Desafiantes[0], "!Você recebeu sua barrra de Bi de volta");
					DoTeleport(Desafiantes[0], 2100, 2100);

					PutItem(Desafiantes[0], &I);
					SendClientMessage(Desafiantes[0], "!Você venceu por W.O.");
				}
...
void RemoveParty(int conn)
{
	...
	else
	{
		...
		if (leader)
		{
			...
			if (conn > 0 && conn < MAX_USER && leader > 0 && leader < MAX_USER) {
				int Ptcount = 0;

				for (int z = 0; z < MAX_PARTY; z++) {
					int ptconn = pMob[leader].PartyList[0];

					if (pMob[leader].PartyList[z] != 0)
						Ptcount++;
				}
				if (Ptcount == 1) {
					Parties[pUser[conn].Keys[39]].Index = 0;
					Parties[pUser[conn].Keys[39]].Chave = 0;
					strncpy(Parties[pUser[conn].Keys[39]].Nick, "", 16);
					pUser[conn].Keys[39] = 0;
					pUser[leader].Keys[39] = 0;
					pUser[conn].Keys[44] = 0;
					pUser[leader].Keys[44] = 0;
				}
			}
		...
		else
		{
			...
			for (int i = 0; i < MAX_PARTY; i++)
			{
				...
				if (partyconn != 0 && partyconn < MAX_USER)
				{
					...
					if (conn > 0 && conn < MAX_USER) {
						Parties[pUser[conn].Keys[39]].Index = novolider;
						strncpy(Parties[pUser[conn].Keys[39]].Nick, pMob[novolider].MOB.MobName, 16);

						int Ptcount = 0;

						for (int z = 0; z < MAX_PARTY; z++) {
							int ptconn = pMob[conn].PartyList[z];
						...
						if (Ptcount == 1) {
							...
							strncpy(Parties[pUser[conn].Keys[39]].Nick, "", 16);
							pUser[conn].Keys[39] = 0;
							pUser[novolider].Keys[39] = 0;
							pUser[conn].Keys[44] = 0;
							pUser[novolider].Keys[44] = 0;
						}
					}
...
void DoRecall(int conn)
{
	...
	if (conn > 0 && conn < MAX_USER) {
		...
		if (pUser[conn].Keys[30] == 1 && ArenaReal.State == 2 && pMob[conn].TargetX >= 140 && pMob[conn].TargetX <= 190 && pMob[conn].TargetY >= 4000 && pMob[conn].TargetY <= 4082) {
			return;
		}
	}
...
void DoTeleport(int mob, int x, int y)
{
	...
	if (ApostaTimer > 5 && (Desafiantes[0] == mob || Desafiantes[1] == mob))
		return;
	...
	if (mob < MAX_USER && mob > 0) {
		...
		if (pUser[mob].Keys[30] == 1 && ArenaReal.State == 2) {
			if (x < 140 || x > 190 || y < 4000 || y > 4082)
				return;
		}
	}
	if (ret)
...
int GetItemCount(int a_iMob, int a_iObject)
{
	int FindCount = 0;
...
int GetSameItem(int a_iMob, STRUCT_ITEM* a_iObject)
{
	for (int i = 0; i < pMob[a_iMob].MaxCarry; i++)
	{
		...
		if (memcmp(&pMob[a_iMob].MOB.Carry[i], &a_iObject, sizeof(STRUCT_ITEM)) != TRUE)
			return i;
	}
