...
void DoComposition(int conn, int NPCID, int COMPID) {
	...
	if (ViewItens[NPCID][COMPID][0] == 0) {
		SendClientMessage(conn, "Composição Inválida");
		return;
	}
	...
	for (int i = 0; i < NITENS; i++) {
		...
		if (CompItens[NPCID][COMPID][i][1] != 61) {
			...
			if (Slot[i] == -1) {
				SendClientMessage(conn, "Você não possui todos os itens necessários");
				return;
			}
		}
		else {
	...
	if (GetFreeSlot(conn) == -1) {
		SendClientMessage(conn, "Não há espaço suficiente no seu inventário");
		return;
	}
	...
	if (sRand > Rate[NPCID][COMPID]) {
		SendClientMessage(conn, strFmt("Composição Falhou! %d/%d", sRand, Rate[NPCID][COMPID]));
...
void ReadNewCompsConfig() {
	...
	sprintf(hQuery, "SELECT * FROM `ncompositorinfo`");
	MYSQL_ROW row;
...
void ReadNewComps() {
	...
	sprintf(hQuery, "SELECT * FROM `newcompositor`");
	MYSQL_ROW row;
	MYSQL* wSQL = pc.wStart();
	MYSQL_RES* result = pc.wRes(wSQL, hQuery);
