...
#pragma region Grupo com Senha
	...
	if (strcmp(m->MobName, "entrar") == 0)
	{
		...
		int size = sizeof(m->String);

		if (sizeof(m->String) > 128 || sizeof(m->String) <= 0)
			return;

		if (strnlen(m->MobName, 16) <= 0 || strnlen(m->String, 128) > 23)
			return;
		...
		if (sscanf_s(m->String, "%15s %6s", TargetName, _countof(TargetName), Password, _countof(Password)))
		{
			...
			{
				SendClientMessage(conn, "Personagem não encontrado");
				return;
			}
			...
			for (int i = 0; i < MAX_PARTY; i++)
			{
				int partyconn = pMob[idx].PartyList[i];
...
#pragma endregion
		else if (strcmp(m->MobName, "asdf") == 0)
		{
			...
			{
				if (pMob[conn].Affect[i].Type)				
				{
					SendClientMessage(conn, strFmt("buff id%d", pMob[conn].Affect[i].Type));
					printf("\nbuff id%d", pMob[conn].Affect[i].Type);
				}
			}
			return;
		}
...
#pragma region InfoPlayer
		else if (!strcmp(m->MobName, "infoplayer"))
		{
			if (strcmp(m->String, "") == 0)
				return;

			int Id = GetUserByName(m->String);

			if (Id != 0)
				ConsultaInfoPlayer(conn, Id);
			else
				SendClientMessage(conn, "Player não está online ou não foi encontrado");
			return;
			}
#pragma endregion
...
#pragma region /tab
	if (strcmp(m->MobName, "tab") == 0)
	{
		...
		if (strcmp(m->String, "AZUL") == 0)
			return;
		if (strcmp(m->String, "LARANJA") == 0)
			return;
		if (strcmp(m->String, "ROXO") == 0)
			return;
		if (strcmp(m->String, "VERDE") == 0)
			return;
