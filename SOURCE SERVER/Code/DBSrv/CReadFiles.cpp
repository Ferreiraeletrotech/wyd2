...
void CReadFiles::ImportItem()
{
	...
	while (1)
	{
		...
		// correção import item
		int Pos = -1, i = 0;

		for (int i = 0; i < MAX_CARGO - 8; i++)
		{
		...
		memcpy(&file.Cargo[Pos], &tempsour, sizeof(STRUCT_ITEM));
...
void CReadFiles::ImportDonate()
{
	...
	while (1)
	{
		...
		if (IdxName > 0 && IdxName < MAX_DBACCOUNT && cFileDB.pAccountList[IdxName].Slot >= 0 && cFileDB.pAccountList[IdxName].Slot <= 3)
		{
			...
			sm.GemaX = GemaX;
			sm.GemaY = GemaY;
			sm.DonateBuyItem = DonateBuyItem;
			sm.MobDonateStore = MobDonateStore;
...
void CReadFiles::WriteRanking()
{
	...
	char classmaster[10][64] = { "SEM CLASSMASTER", "M", "A", "C", "C", "SC", "HC", "LC", "SH", "LGC" };
	...
	for (int i = RankPos::FIRST, pos = 1; i > RankPos::LAST; i--)
	{
		...
		{
			fprintf(fp, "%d %s %s %s %d %d \n", rankingSystem.grindRanking.getElement(i)->Level, rankingSystem.grindRanking.getElement(i)->Name,
